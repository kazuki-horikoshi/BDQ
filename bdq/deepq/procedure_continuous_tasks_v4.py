...
def learn_continuous_tasks(env,
    ...
    if epsilon_greedy:
        approximate_num_iters = 2e6 / 4 # change!!! 365 * 96
        exploration = PiecewiseSchedule([(0, 1.0),
                                        (approximate_num_iters / 50, 0.1), # start training? 
                ...
                if eval_std == 0.0: # check!!!
                    ...
                    for index in range(len(actions_greedy)): 
                        ...
                        while out_of_range_action:
                            ...
                            a_idx_stoch = np.rint((a_stoch + high[index]) / actions_range[index] * num_action_grains) # why + high?
    ...
    rewards_save_name = "daily_rewards"
    # save the episode/daily rewards into csv
    rewards_df = pd.DataFrame({'episode_rewards':episode_rewards})
    rewards_df.to_csv("/home/ylei/Desktop/Github/action-branching-agents/agents/bdq/trained_models/emulator-v0/"+rewards_save_name+"5.13.csv")

    obs_save_name = "observations"
    # save the record
    ...
    obs_df.to_csv("/home/ylei/Desktop/Github/action-branching-agents/agents/bdq/trained_models/emulator-v0/"+obs_save_name+"5.13.csv")
