...
def learn_continuous_tasks(env,
    ...
    # set the parameters for env, this can be done during gym.make() in later gym versions
    env.eta = eta
    env.occStatus = occStatus

    # since FMU env cannot be not pickled, all the "env." related infomation should be avoid
    observation_space = env.observation_space.shape
    def make_obs_ph(name):
        #return U.BatchInput(env.observation_space.shape, name=name)
        return U.BatchInput(observation_space, name=name)

    num_action_grains = num_actions_pad - 1
    num_action_dims = env.action_space.shape[0] # for continuous box space
    #num_action_dims = env.action_space.shape # for multi discrete space
    ...
    if epsilon_greedy:
        approximate_num_iters = 2e6 / 4 # change if using e-greedy!!!
    ...
    def evaluate(step, episode_number):
        ...
        for eval_episode in range(n_eval_episodes):
            if eval_episode == 0: obs = env.reset()
            #done = False
            #while not done:
            for s in range(steps):
                ...
                if eval_std == 0.0: # check!!!
                    #action = actions_greedy
                    action = action_idxes # action index will convert to real action within emulator
                else:
                    action = []
                    for index in range(len(actions_greedy)): 
                        ...
                        while out_of_range_action:
                            ...
                            a_idx_stoch = np.rint((a_stoch + high[index]) / actions_range[index] * num_action_grains) # this only works for [-1,+1] action space
                            if a_idx_stoch >= 0 and a_idx_stoch < num_actions_pad:
                                #action.append(a_stoch)
                                action.append(a_idx_stoch)
                                out_of_range_action = False

                # Step
                horizon = int(eval_episode*24*3600 + s*3600/4 + 6.5*3600)
                #print ("action", action, action_idxes, eval_episode, s)
                obs, rew, done, _ = env.step(action, horizon)
                eval_reward_sum += rew
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with open("{}/{}_{}_{}.csv".format(results_dir, method_name, time_stamp, env_name), "w") as fw: 
            ...
            while True:
                ...
                action_idxes = np.array(act(np.array(obs_normal)[None], update_eps=exploration.value(t))) # use normalized obs
                ...
                if not epsilon_greedy: # Gaussian noise
                    ...
                    for index in range(len(actions_greedy)): 
                        ...
                        while out_of_range_action:
                            ...
                            a_idx_stoch = np.rint((a_stoch + high[index]) / actions_range[index] * num_action_grains) # works for [-1,1] range
                            # a_idx_stoch = np.rint((a_stoch - low[index]) / actions_range[index] * num_action_grains)
                ...
                t_step = t % steps # time steps for horizon, iteration between 0 and 95
                horizon = int(len(episode_rewards)*24*3600 + t_step*3600/4 + 6.5*3600) # calc the next control timestep
                new_obs, rew, done, info = env.step(action_idxes, horizon)
                new_obs_normal = (new_obs - low_obs) / obs_range # normalize obs to 0-1 scale
                ...
                replay_buffer.add(obs_normal, action_idxes, rew, new_obs_normal, float(done)) # use normalized obs
                obs = new_obs
                obs_normal = new_obs_normal

                reward_sum += rew
                
                num_episodes = len(episode_rewards)
                print("episode:", num_episodes, "steps:", t_step)
                # one day (t = steps = 96) is one episode
                if (t+1) % steps == 0:
                    #obs = env.reset()
                ...
                if (t+1) == steps and print_freq is not None and len(episode_rewards) % print_freq == 0:
                ...
                if (t+1) == steps and num_episodes % eval_freq == 0:
                    pass
                    #evaluate(t+1, num_episodes) # maybe evaluate on the actual building data after implementation?
                    #obs = env.reset()
    ...
    rewards_save_name = "/daily_rewards_"+results_version
    # save the episode/daily rewards into csv
    rewards_df = pd.DataFrame({'episode_rewards':episode_rewards})
    rewards_df['weekly_moving_rewards'] = rewards_df.iloc[:,0].rolling(window=7).mean() # weekly moving average of rewards
    rewards_df['monthly_moving_rewards'] = rewards_df.iloc[:,0].rolling(window=30).mean() # monthly moving average of rewards
    rewards_df.to_csv(results_dir+rewards_save_name+".csv")

    obs_save_name = "/observations_"+results_version
    # save the record
    obs_name = ["Room T", "vAir1", "vAir2", "vAir3", "vAir4", "Outdoor T", "Solar Radiation", "RH", "hour", "occupancyFlag", "Time", "Air Flow", "Energy", "P1", "P2", "P3", "P4", "P5", "P6", "P7", "P8", "comfort penalty", "Reward" , "Act_T", "Act_F1", "Act_F2", "Act_F3", "Act_F4"]
    #obs_df = pd.DataFrame(observations, index = np.array(timeStamp), columns = obs_name)
    obs_df = pd.DataFrame(observations, index = timeStamp, columns = obs_name)
    # show only testing period if turncate variable is set to True
    if truncate == True: obs_df = obs_df.truncate(before="2020-01-01")
    obs_df.to_csv(results_dir+obs_save_name+".csv")

    # print out the energy and comfort info
    obs_df_occ = obs_df.drop(obs_df[obs_df['occupancyFlag'] == 0].index)
    energy = obs_df['Energy'].mean()
    comfort = obs_df['comfort penalty'].mean()
    energy_occ = obs_df_occ['Energy'].mean()
    comfort_occ = obs_df_occ['comfort penalty'].mean()
    print("Total Energy: {}\nTotal Comfort: {}\nOcc Energy: {}\nOcc Comfort: {}".format(energy,comfort,energy_occ,comfort_occ))
